...
import AttemptReview from '@/components/AttemptReview' // <--- USAMOS EL COMPONENTE COMPARTIDO

interface PageProps {
  params: Promise<{ courseId: string; studentId: string; attemptId: string }>
}

export default async function TeacherReviewPage({ params }: PageProps) {
  const { courseId, studentId, attemptId } = await params
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-fade-in pb-20">
      ...
      {/* Header con navegación de profesor */}
      <div>
        <Link 
            ...
            <div className="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-xl border border-slate-200">
                <div className="text-right">
                    ...
                    <p className={clsx("text-2xl font-black", 
                        attempt.mastery_level === 'Dominado' ? "text-green-600" : "text-slate-800"
                    )}>
